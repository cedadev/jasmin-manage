...
def project_action(viewset_method=None, serializer_class=serializers.Serializer):
    ...
    action_decorator = action(
        detail=True, methods=["POST"], serializer_class=serializer_class
    )
...
def project_action_with_comment(viewset_method):
    ...
    def wrapper(viewset, project, data):
        ...
        with transaction.atomic():
            ...
            project.comments.create(content=data["comment"], user=viewset.request.user)
...
class ProjectViewSet(
    ...
    def events(self, request, pk=None):
        ...
        events = Event.objects.filter(
            aggregate__aggregate_ctype=content_type, aggregate__aggregate_id=project.pk
    ...
    def submit_for_review(self, project, data):
        ...
        requirement_counts = Requirement.objects.filter(
            service__project=project
        ).aggregate(
            requested_count=models.Count(
                "status", filter=models.Q(status=Requirement.Status.REQUESTED)
            ),
            rejected_count=models.Count(
                "status", filter=models.Q(status=Requirement.Status.REJECTED)
            ),
            approved_count=models.Count(
                "status", filter=models.Q(status=Requirement.Status.APPROVED)
            ),
        )
        ...
        if (requested_count + approved_count) < 1:
            raise Conflict(
                "Project has no requirements to review.", "no_requirements_to_review"
            )
    ...
    def submit_for_provisioning(self, project, data):
        ...
        unapproved = Requirement.objects.filter(
            service__project=project, status__lt=Requirement.Status.APPROVED
        )
        ...
        approved = Requirement.objects.filter(
            service__project=project, status=Requirement.Status.APPROVED
        )
...
class ProjectCollaboratorsViewSet(
    BaseViewSet, mixins.ListModelMixin, viewsets.GenericViewSet
):
    """
...
class ProjectInvitationsViewSet(
    mixins.ListModelMixin, mixins.CreateModelMixin, viewsets.GenericViewSet
):
    """
